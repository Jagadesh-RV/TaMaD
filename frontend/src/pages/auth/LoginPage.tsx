  ...
  return (
    <div className="auth-page">
      <div className="auth-card">
        ...
        <button type="button" onClick={handleGoogleSignIn} disabled={isLoading} className="btn-secondary auth-provider-button">
          <span className="auth-google-mark">G</span> Continue with Google
        </button>
        <Link to="/phone-login" className="auth-phone-link">Use your phone number instead</Link>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div>
            ...
            <div className="relative">
              <input {...register('password')} type={showPassword ? 'text' : 'password'} className="input-field pr-12" placeholder="••••••••" autoComplete="current-password" />
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-3 text-gray-400 hover:text-gray-600 transition-colors" aria-label={showPassword ? 'Hide password' : 'Show password'}>
          ...
          <div className="flex items-center justify-between pt-2">
            <label className="flex items-center text-sm text-gray-600 cursor-pointer hover:text-gray-900 transition-colors">
              <input {...register('rememberMe')} type="checkbox" className="w-4 h-4 rounded border-gray-300 bg-white text-primary focus:ring-primary focus:ring-offset-background mr-3 cursor-pointer" /> Remember me
            </label>
          ...
          <button type="submit" disabled={isLoading} className="btn-primary mt-4 flex items-center justify-center gap-2 group">
            {isLoading ? <Loader2 className="animate-spin" size={24} /> : <>Sign In to TaMaD <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" /></>}
          </button>
        </form>
        <div className="mt-10 text-center border-t border-border pt-8"><p className="text-gray-600">Don't have an account? <Link to="/register" className="font-bold text-primary hover:text-primary-hover transition-colors ml-1">Create an account</Link></p></div>
      </div>
    </div>
  );
}
