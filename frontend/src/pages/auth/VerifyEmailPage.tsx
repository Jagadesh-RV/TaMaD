...
export default function VerifyEmailPage() {
  ...
  const continueToDashboard = async () => { try { await auth.currentUser?.reload(); const user = await completeFirebaseSignIn(false); if (!user.emailVerified) return toast.error('Your email is not verified yet.'); navigate('/'); } catch (error: any) { toast.error(error.message || 'Unable to refresh verification status.'); } };
